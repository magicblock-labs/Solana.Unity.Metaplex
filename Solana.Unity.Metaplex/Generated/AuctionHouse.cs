...
using Solana.Unity.Metaplex.AuctionHouse.Accounts;
using Solana.Unity.Metaplex.AuctionHouse.Errors;
using Solana.Unity.Metaplex.AuctionHouse.Program;
using Solana.Unity.Metaplex.AuctionHouse.Types;

// ReSharper disable once CheckNamespace

namespace Solana.Unity.Metaplex.AuctionHouse
{
    namespace Accounts
    {
    ...
    public partial class AuctionHouseClient : TransactionalBaseClient<AuctionHouseErrorKind>
    {
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<Accounts.Auctioneer>>> GetAuctioneersAsync(string programAddress, Commitment commitment = Commitment.Finalized)
        {
            var list = new List<Solana.Unity.Rpc.Models.MemCmp>{new Solana.Unity.Rpc.Models.MemCmp{Bytes = Accounts.Auctioneer.ACCOUNT_DISCRIMINATOR_B58, Offset = 0}};
            var res = await RpcClient.GetProgramAccountsAsync(programAddress, commitment, memCmpList: list);
            if (!res.WasSuccessful || !(res.Result?.Count > 0))
                return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<Accounts.Auctioneer>>(res);
            List<Accounts.Auctioneer> resultingAccounts = new List<Accounts.Auctioneer>(res.Result.Count);
            resultingAccounts.AddRange(res.Result.Select(result => Accounts.Auctioneer.Deserialize(Convert.FromBase64String(result.Account.Data[0]))));
            return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<Accounts.Auctioneer>>(res, resultingAccounts);
        }
        ...
        public async Task<Solana.Unity.Programs.Models.AccountResultWrapper<Accounts.Auctioneer>> GetAuctioneerAsync(string accountAddress, Commitment commitment = Commitment.Finalized)
        {
            var res = await RpcClient.GetAccountInfoAsync(accountAddress, commitment);
            if (!res.WasSuccessful)
                return new Solana.Unity.Programs.Models.AccountResultWrapper<Accounts.Auctioneer>(res);
            var resultingAccount = Accounts.Auctioneer.Deserialize(Convert.FromBase64String(res.Result.Value.Data[0]));
            return new Solana.Unity.Programs.Models.AccountResultWrapper<Accounts.Auctioneer>(res, resultingAccount);
        }
        ...
        public async Task<SubscriptionState> SubscribeAuctioneerAsync(string accountAddress, Action<SubscriptionState, Solana.Unity.Rpc.Messages.ResponseValue<Solana.Unity.Rpc.Models.AccountInfo>, Accounts.Auctioneer> callback, Commitment commitment = Commitment.Finalized)
        {
            SubscriptionState res = await StreamingRpcClient.SubscribeAccountInfoAsync(accountAddress, (s, e) =>
            {
                Accounts.Auctioneer parsingResult = null;
                if (e.Value?.Data?.Count > 0)
                    parsingResult = Accounts.Auctioneer.Deserialize(Convert.FromBase64String(e.Value.Data[0]));
                callback(s, e, parsingResult);
            }, commitment);
            return res;
        }
