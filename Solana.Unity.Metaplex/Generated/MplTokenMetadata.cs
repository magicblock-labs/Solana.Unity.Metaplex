...
using Solana.Unity.Metaplex.MplTokenMetadata;
using Solana.Unity.Metaplex.MplTokenMetadata.Accounts;
using Solana.Unity.Metaplex.MplTokenMetadata.Errors;
using Solana.Unity.Metaplex.MplTokenMetadata.Program;
using Solana.Unity.Metaplex.MplTokenMetadata.Types;

// ReSharper disable once CheckNamespace

namespace Solana.Unity.Metaplex.MplTokenMetadata
{
    namespace Accounts
    {
        ...
        public partial class Metadata
        {
            ...
            public Types.Uses Uses { get; set; }
            ...
            public static Metadata Deserialize(ReadOnlySpan<byte> _data)
            {
                ...
                if (_data.GetBool(offset++))
                {
                    offset += Types.Collection.Deserialize(_data, offset, out var resultCollection);
                    result.Collection = resultCollection;
                }

                if (_data.GetBool(offset++))
                {
                    offset += Types.Uses.Deserialize(_data, offset, out var resultUses);
